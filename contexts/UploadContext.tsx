...
interface UploadState {
  ...
  name: string;
  progress: number;
  status: 'uploading' | 'completed' | 'failed' | 'paused' | 'encrypting' | 'processing';
  size: number;
  uploadedSize: number;
  error?: string;
  startTime?: number;
...
interface UploadContextType {
  uploads: UploadState[];
  addUpload: (upload: Omit<UploadState, 'startTime' | 'estimatedTimeRemaining'>) => void;
...
export function UploadProvider({ children }: { children: React.ReactNode }) {
  ...
  const addUpload = useCallback((upload: Omit<UploadState, 'startTime' | 'estimatedTimeRemaining'>) => {
  ...
  const updateUpload = useCallback((id: string, update: Partial<UploadState>) => {
    setUploads(prevUploads => {
      ...
      if (update.uploadedSize && update.uploadedSize > currentUpload.uploadedSize) {
  ...
  const cancelUpload = useCallback((id: string) => {
    ...
    if (upload) {
      updateUpload(id, { status: 'failed', error: 'Upload cancelled by user' });
    }
  ...
  const getTotalProgress = useCallback(() => {
    const activeUploads = uploads.filter(u => 
      u.status === 'uploading' || u.status === 'encrypting' || u.status === 'processing'
    );
    
    const total = activeUploads.reduce((sum, upload) => sum + upload.size, 0);
    const completed = activeUploads.reduce((sum, upload) => sum + upload.uploadedSize, 0);

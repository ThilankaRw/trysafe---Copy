...
export default async function handler(
  ...
  try {
    ...
    if (!session || !session.user) {
      console.warn("Unauthorized DELETE request to /api/files/delete");
      return res.status(401).json({ message: "Unauthorized" });
    }
  } catch (authError) {
    console.error("Authentication error in /api/files/delete:", authError);
  ...
  try {
    console.log(
      `[API /api/files/delete] Attempting to delete file ${fileId} for user ${userId}`
    );
    ...
    await prisma.$transaction(async (tx) => {
      // If the file has chunks, delete them first
      if (file.chunks && file.chunks.length > 0) {
        // Delete the chunks from S3
        // This would typically call your S3 service
        for (const chunk of file.chunks) {
          // Placeholder for actual S3 deletion logic
          console.log(`Deleting chunk with S3 key ${chunk.s3Key}`);
          // await s3Client.deleteObject({ Bucket: process.env.S3_BUCKET, Key: chunk.s3Key });
        }
    ...
    console.log(`[API /api/files/delete] Successfully deleted file ${fileId}`);
    ...
    console.error(
      `[API /api/files/delete] Error deleting file ${fileId}:`,
      error
    );
    return res
      .status(500)
